...
public func <<? <T>(left: inout T?, right: Mapping) throws where T: Decodable {
    try right.map.decodeIfPresent(object: &left, with: right.key, options: right.options)
}
...
public func ?>> <T>(left: T?, right: Mapping) throws where T: Encodable {
    try right.map.encodeIfPresent(object: left, with: right.key, options: right.options)
}
